...
@Component({
  ...
  template: `<mat-card class="example-card">
    <mat-card-title>Consultar datos de Usuarios</mat-card-title>
    <mat-card-content>
      <button mat-button color="primary" routerLink="/editarusuario/0">
        ...
        <ng-container matColumnDef="actions">
          ...
          <td mat-cell *matCellDef="let element">
            <button
              mat-button
              color="primary"
              routerLink="/editarusuario/{{ element.codigo }}"
            >
...
export class ConsultarUsuarioComponent implements OnInit {
  ...
  consultar = () => {
    axios
      .get<any>('http://localhost:8080/PROYECTO-REST/rest/usuario/list')
      .then((resultado) => {
  ...
  borrar = (codigo: number) => {
    axios
      .delete<any>(
        `http://localhost:8080/PROYECTO-REST/rest/usuario/delete/${codigo}`
      )
      .then((resultado) => {
        if (resultado.data['success'] == true) {
          this.consultar();
