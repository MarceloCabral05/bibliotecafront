...
@Component({
  ...
  template: `<mat-card class="example-card">
    <mat-card-title>Consultar datos de Ciudades</mat-card-title>
    <mat-card-content>
      <button mat-button color="primary" routerLink="/editarciudad/0">
        ...
        <ng-container matColumnDef="actions">
          ...
          <td mat-cell *matCellDef="let element">
            <button
              mat-button
              color="primary"
              routerLink="/editarciudad/{{ element.codigo }}"
            >
...
export class ConsultarCiudadComponent implements OnInit {
  ...
  borrar = (codigo: number) => {
    axios
      .delete<any>(
        `http://localhost:8080/PROYECTO-REST/rest/ciudad/delete/${codigo}`
      )
      .then((resultado) => {
        if (resultado.data['success'] == true) {
          this.consultar();
